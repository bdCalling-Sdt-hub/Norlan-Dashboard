...
const Category = () => {
    ...
    const handleSubmit = async (values) => {
        try {
            ...
            if(value){
                await updateCategory({id: value?._id, updatedData: formData}).unwrap().then(({ status, message }) => {
                    if (status) {
            ...
            await createCategory(formData).unwrap().then(({ status, message }) => {
                if (status) {
                    toast.success(message);
                    setOpen(false);
                    refetch();
    ...
    const handleDelete=async(id)=>{
        try {
            await deleteCategory(id).unwrap().then(({status, message})=>{
                if (status) {
                    toast.success(message);
                    refetch()
                }
    ...
    return (
        <div>
            ...
            <div className='flex items-center justify-between mb-6'>
                ...
                <button onClick={()=> setOpen(true)} className='bg-primary text-white h-10 px-4 rounded-md'>Add Category</button>
            </div>

            <Table dataSource={categories?.data} pagination={false} columns={columns}  />
